<template>
  <div :class="['player_card', `theme-${theme}`, { active: props.active }]" @click="manageClick">
    <div class="image">
      <svg v-if="isFemale" version="1.1" baseProfile="tiny" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
        xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 64 64" overflow="visible"
        xml:space="preserve">
      ...
      <svg v-else version="1.1" baseProfile="tiny" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
        xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 64 64" overflow="visible"
        xml:space="preserve">
...
import { computed, ref, inject, defineComponent, markRaw } from 'vue'
import { Iplayer } from './interfaces'
import PerformanceDonutGraph from './PerformanceDonutGraph.vue';

const theme = inject('theme', 'light')
const props = defineProps<{ player: Iplayer, active: boolean }>()
const emit = defineEmits(['selectedPlayer'])
const showDetails = ref(false)

const player = computed(() => props?.player)
const posicion = computed(() => player.value?.position || 'unknown')
const isFemale = computed(() => player.value?.gender === 'female')
      ...
      return () => (
        <div class="player-details">
          ...
          <div class="details">
            ...
            <label>Position: <span>{player.value.position}</span></label>
            {player.value.number && <label >Number: <span>{player.value.number}</span></label>}
            {player.value?.height && <label>Height: <span>{player.value.height}</span></label>}
            {player.value?.handPreference && <label>Hand preference: <span>{player.value.handPreference}</span></label>}
          </div>
          <div class="statistics">
            <h3>Statistics</h3>
            <PerformanceDonutGraph stats={player.value?.seasonStats} />
          </div>
        </div>
      )
    }
  })
  emit('selectedPlayer', markRaw(comp), player.value.id)
}
</script>
<style>
.player-details {
...
.player-details .image img {
  height: 100%;
  width: 100%;
  object-fit: contain;
}

.player-details .image svg path,
.player-details .image svg circle,
.player-details .image svg rect {
  fill: var(--text-color);
}
...
.player_card:not(:first-child):not(:last-child).active {
  margin-left: 32px;
}
